...
function spawnEnemy(type, x, y, camp, raceId) {
  ...
  G.enemies.push({ type, camp, def: R ? d : undefined, x, y, hx: x, hy: y, hp: Math.round(d.hp * mul), maxHp: Math.round(d.hp * mul), mul, cd: 0, dir: 0, swing: 0, flash: 0, walk: 0, wt: 0, wx: x, wy: y, aggro: false });
}
...
function update(dt) {
  ...
  for (const site of ['camp1', 'fort', 'legion']) {
  ...
  if (G.autoT >= 2.2) { G.autoT = 0; autoManage(); }
...
function render() {
  ...
  if (ISLAND && G.islandMates && G.islandMates.length) {
    for (const m of G.islandMates) {
      if (!m.pos || !vis(m.pos.x, m.pos.y)) continue;
